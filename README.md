...
Zuul 서버에서는 단일 VM 위에 Docker Compose로 Zuul 14.x 를 올린다. 주요 컨테이너:
...
- `zuul-test-zk-1` / `zuul-test-mysql-1` — ZooKeeper(상태) + MariaDB(빌드 히스토리).
- `zuul-test-log-server-1` — nginx, 잡 로그 서빙 (포트 8088).

zuul.conf에서 GitHub 연결은 `exclude_forks=false` 로 fork 레포도 추적할 수 있게 한다. 단, 새 레포를 추가할 때는 ZK 캐시를 비워야 반영된다.

### Config-project 격리 원칙

**Zuul 인스턴스 1개 = config-project fork 1개 = 단일 브랜치(보통 `main`)**.

같은 config-project를 여러 Zuul 인스턴스가 공유하면 양쪽 모두 동작 불가가 된다 — Zuul 14.x 의 tenant parser는 `load-branch` / `include-branches` 옵션과 무관하게 모든 브랜치의 `zuul.d/` 를 cat 하기 때문에, 한 인스턴스에서 secrets를 재암호화하면 다른 인스턴스가 priming 단계에서 `Decryption failed` 로 깨진다. 새 테스트 환경이 필요하면 fork를 새로 떠서 그 fork를 가리키게 한다 ([docs/zuul-server-bootstrap.md §Why a dedicated config-project fork](docs/zuul-server-bootstrap.md#why-a-dedicated-config-project-fork)).
...
새 환경에서 secret을 처음 만들거나 키 회전할 때는 [`tools/encrypt_secret.py`](tools/encrypt_secret.py) 를 사용한다. 컨테이너 안에서 공개키를 fetch 하는 `--container` 모드를 권장 — 컨테이너 startup 직후엔 외부 `:9000/api/.../key/...pub` 가 stale 캐시를 반환할 수 있어 암호화한 secret이 해독 안 되는 함정이 있다.
...
### 부트스트랩 자료에 박힌 평문 자격증명

[`bootstrap/`](bootstrap/) 의 dev-default 값들 — 운영 전환 시 **반드시** 교체.

| 위치 | 값 (dev default) | 의미 |
|------|-----------------|------|
| [`bootstrap/docker-compose.yaml`](bootstrap/docker-compose.yaml) `mysql` 서비스 | `MYSQL_ROOT_PASSWORD: rootpassword` | MariaDB root 비밀번호 |
| 〃 | `MYSQL_PASSWORD: secret` | Zuul용 MariaDB 사용자 비밀번호 |
| `bootstrap/etc/zuul/zuul.conf.example` | `[keystore] password=secret` | ZK 안에 저장된 프로젝트 비밀키 암호화 |
| 〃 | `[auth zuul_operator] secret=<ZUUL_OPERATOR_SECRET>` | REST API enqueue 토큰 서명용 HS256 비밀 (`zuul create-auth-token`) |
| 〃 | `webhook_token=<GITHUB_WEBHOOK_TOKEN>` | GitHub App 생성 시 설정한 webhook 비밀 |
| 〃 | `app_id=<GITHUB_APP_ID>` | 운영 GitHub App ID로 교체 |
| `~/zuul-test/etc/zuul/github_app.pem` | (테스트 App의 .pem) | 운영 App `.pem` 으로 교체 |
| `~/zuul-test/keys/{nodepool,zuul}_rsa` | (테스트 환경 SSH 키) | 운영 환경에서 새로 생성 + 워커 `authorized_keys` 재배포 |

`docker-compose.yaml` 의 mysql 비밀번호를 바꿀 때는 같은 값을 `zuul.conf` 의 `[database] dburi` 도 따라 바꿔야 한다.
...
- [`tools/encrypt_secret.py`](tools/encrypt_secret.py) — Zuul secret 암호화 헬퍼 (RSA-OAEP-SHA1 chunking + stale-cache 회피)
