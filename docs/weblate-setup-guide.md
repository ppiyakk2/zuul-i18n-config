...
[keys]
https://weblate.example.com/api/ = wlu_xxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
| 파라미터 | 값 | 설명 |
|---------|---|------|
| `category` | 카테고리 API URL | 예: `http://weblate.example.com/api/categories/1/` |
...
[keys]
https://weblate.example.com/api/ = wlu_xxxxx
```
...
| Zuul 서버 | <ZUUL_HOST_IP> (ubuntu, SSH key: ~/.ssh/your-zuul-key.pem) |
| Job 실행 노드 | <WORKER_HOST_IP> / <WORKER_INTERNAL_IP> (Ubuntu 24.04) |
| 소스 경로 (노드) | ~/src/github.com/<your-org>/contributor-guide (Zuul이 자동 동기화) |
