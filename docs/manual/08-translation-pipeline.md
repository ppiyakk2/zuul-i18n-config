...
───────────────────────────────────────────────
Project                          Git 프로젝트
  = contributor-guide              = <your-org>/contributor-guide
...
# 3. 빌드 결과 확인
curl -s "http://<ZUUL_HOST_IP>:9000/api/tenant/i18n-test/builds?limit=1" | python3 -m json.tool
...
1. **빌드 상태 확인**: Web UI (`http://<ZUUL_HOST_IP>:9000/`) 또는 API
2. **로그 확인**: 로그 서버 (`http://<ZUUL_HOST_IP>:8088/<uuid>/job-output.txt`)
3. **노드 직접 확인**: `ssh ubuntu@<WORKER_HOST_IP>`
