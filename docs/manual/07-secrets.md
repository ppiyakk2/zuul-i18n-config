...
curl -s "http://133.186.247.137:9000/api/tenant/i18n-test/key/ppiyakk2/zuul-i18n-config.pub"
```
...
zuul encrypt --tenant i18n-test \
    --project ppiyakk2/zuul-i18n-config \
    --secret weblate_api_credentials \
    --field url \
    --infile <(echo -n "https://weblate.printf.kr/api/")
```
...
encrypted = cipher.encrypt(b"https://weblate.printf.kr/api/")
