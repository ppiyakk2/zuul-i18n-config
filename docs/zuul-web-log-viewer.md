  ...
  zuul_return:
    data:
      zuul:
        ...
        artifacts:                                          # ← 이 부분이 핵심
          - name: "Zuul Manifest"
            url: "http://133.186.247.137:8088/{{ zuul.build }}/zuul-manifest.json"
            metadata:
              type: zuul_manifest
```
...
1. **빌드 API에 artifacts가 있는가?**
   ```
   curl http://133.186.247.137:9000/api/tenant/i18n-test/builds?limit=1 | jq '.[0].artifacts'
   ```
   `zuul_manifest` 타입이 있어야 Logs 탭이 동작한다.

2. **manifest URL에 접근 가능한가?**
   ```
   curl http://133.186.247.137:8088/{build_uuid}/zuul-manifest.json
   ```

3. **`.gz` 파일의 Content-Type이 올바른가?**
   ```
   curl -I http://133.186.247.137:8088/{build_uuid}/job-output.json.gz
   ```
...
4. **CORS 헤더가 있는가?**
   ```
   curl -I http://133.186.247.137:8088/{build_uuid}/zuul-manifest.json
   ```
